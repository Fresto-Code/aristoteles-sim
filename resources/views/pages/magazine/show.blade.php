                                ...
                                <div class="col">
                                    <div class="card-profile-stats d-flex justify-content-center">
                                        @if ($magazine->moderation_status == 'draft')
                                            <form class="m-1" action="/magazine/{{ $magazine->id }}/approve"
                                                method="post">
                                                @method('PATCH')
                                                @csrf
                                        ...
                                        @else
                                            <form class="m-1" action="/magazine/{{ $magazine->id }}/cancel"
                                                method="post">
                                                @method('PATCH')
                                                @csrf
                    ...
                    <div class="card-body">
                        @if ($magazine->content === '' || $magazine->content === null || $magazine->moderation_status == 'published')
                            <object data="your_url_to_pdf" type="application/pdf">
                                <iframe src="{{ $magazine->url }}" height="1000px"
                                    style=" display:block;
                                width:100%;"></iframe>
                            </object>
                        @elseif (($magazine->content !== '' || $magazine->content !== null) && $magazine->moderation_status == 'draft')
                            <form id="editor_form" onsubmit="return upload(this);" method="POST"
                                action="{{ route('magazine/inline/editor') }}" enctype="multipart/form-data">
                                @csrf
                                <div class="form-group">
