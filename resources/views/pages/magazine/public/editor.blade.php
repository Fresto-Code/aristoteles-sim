...
<div class="header bg-gradient-primary py-7 py-lg-8">
    <div class="container bg-white p-5 rounded">
        <form id="editor_form" onsubmit="return upload(this);" method="POST" action="{{ route('magazine/inline/editor') }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                <div class="input-group mb-3">
                    <input class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }} border-1" placeholder="{{ __('judul') }}" type="text" name="title" value="{{ old('title') }}" required autofocus>
                </div>
                @if ($errors->has('title'))
            ...
            <div class="form-group{{ $errors->has('writenMagazine') ? ' has-danger' : '' }}">
                <input name="writenMagazine" type="hidden">
                <div class="bg-white" id="editor" style="height: 700px;"></div>
                @if ($errors->has('writenMagazine'))
                <span class="invalid-feedback" style="display: block;" role="alert">
                    <strong>{{ $errors->first('writenMagazine') }}</strong>
                </span>
                @endif
            </div>
